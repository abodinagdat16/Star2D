...
public class CircleItem extends CircularImageView implements PlayerItem {
	Body body;
	PropertySet<String,Object> propertySet;
	PlayerView player;
	float height,scale=1,width;
	public ElementEvent elementEvent;
	float[] offset = new float[]{0,0};//collider x,y
	public CircleItem(Context context,PropertySet<String,Object> props,PlayerView pv,ElementEvent event){
		super(context);
		...
		setOnClickListener(new View.OnClickListener(){
			...
			public void onClick(View arg0) {
				if(elementEvent!=null) elementEvent.onClick(CircleItem.this);
			}
		});
	}
	...
	private void createBody(){
		...
		width = propertySet.getFloat("radius");
		...
		if(!propertySet.getString("type").equals("UI")){
			...
			shape.setRadius(propertySet.getFloat("Collider Radius")*player.worldScale);
	...
	public void update(){
		if(body!=null){
			if(scale!=player.scale) {
				scale = player.scale;
				setLayoutParams(new FrameLayout.LayoutParams((int)(2*scale*width),(int)(2*scale*height)));
			}
			float x= body.getPosition().x*(1/player.worldScale)+offset[0]-(getMeasuredWidth()*player.scale/2),
			y= body.getPosition().y*(1/player.worldScale)+offset[1]-(getMeasuredHeight()*player.scale/2),
	...
	public View getClone(String newName) {
		...
		return new CircleItem(getContext(),set,player,elementEvent);
	}
