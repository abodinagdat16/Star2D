...
public class FilesManagerActivity extends AppCompatActivity {
	...
	private Intent fp_imgs = new Intent(Intent.ACTION_GET_CONTENT);
	private Intent fp_files = new Intent(Intent.ACTION_GET_CONTENT);
	private Intent fp_sounds = new Intent(Intent.ACTION_GET_CONTENT);
	private Intent intent = new Intent();
	private AlertDialog.Builder dll;
	private MediaPlayer mp;
	ActivityResultLauncher imgs_picker,files_picker,sounds_picker;
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		initialize(_savedInstanceState);
			...
			imgs_picker = registerForActivityResult(new ActivityResultContracts.OpenMultipleDocuments(),new ActivityResultCallback<List<Uri>>(){
				@Override
			...
			files_picker = registerForActivityResult(new ActivityResultContracts.OpenMultipleDocuments(),new ActivityResultCallback<List<Uri>>(){
				@Override
				public void onActivityResult(List<Uri> uriList) {
				    if(uriList==null) return;
					String p="";
			...
			public void onClick(View _view) {
				if (types_x.contains("anims") || types_x.contains("images")) {
					...
					imp.setOnClickListener(new View.OnClickListener(){
						...
						public void onClick(View _view){
							if (types_x.contains("images")) {
								if(Build.VERSION.SDK_INT>=30) imgs_picker.launch(new String[]{"image/*"});
					...
					if (types_x.contains("files")) {
						...
						if (types_x.contains("sounds")) {
							if(Build.VERSION.SDK_INT>=30) sounds_picker.launch(new String[]{"audio/*"});
	...
	private void initializeLogic() {
		linear2.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns(25, 1, 0xFFFFB300, 0xFF3C3C3C));
		add.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns(25, 0xFF0078FE));
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			renameLin.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns(15, 0, 0xFFFFB300, 0xFF515151));
			...
			renameLin.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View _view) {
					...
					add.setOnClickListener(new View.OnClickListener(){
						@Override
						public void onClick(View _view){
							if ((!(Uri.parse(_data.get(_position).get("file").toString()).getLastPathSegment().toLowerCase().equals(name.getText().toString().toLowerCase())))) {
