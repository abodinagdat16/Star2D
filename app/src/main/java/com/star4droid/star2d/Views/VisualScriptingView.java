...
public class VisualScriptingView extends LinearLayout {
	...
	private HashMap<String, Object> codesMap = new HashMap<>();
	private boolean isArabic = false;
	public String imgsPath = "",hintsLst="",json_path="",code_path="";
	TextView targetText;
	
	private ArrayList<String> hintsL = new ArrayList<>();
	private ArrayList<HashMap<String, Object>> imgsLm = new ArrayList<>();
	private ArrayList<String> interals = new ArrayList<>();
	...
	private void initializeLogic() {
		...
		add.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns(90, 0xFFF44336));
		show.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns(90, 0xFFF44336));
		/*
		try {
			isArabic = save_sh.getString("isArabic", "").equals("true");
			} catch (Exception e) {
		...
		*/
		...
		try{
			java.io.InputStream nodesIn = getContext().getAssets().open("java/nodes.java");
			int nodesSi = nodesIn.available();
			byte[] nodesBu = new byte[nodesSi];
			nodesIn.read(nodesBu);
			nodesIn.close();
			nodes = new String(nodesBu, "UTF-8");
			}catch(Exception e){
		...
		for(String str1:fls){
			...
			if (!fis.equals("")) {
				...
				lm = new Gson().fromJson(fis, new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
				if(lm.size()>0) nodes = nodes+"\nsplit\n--"+Uri.parse(str1).getLastPathSegment();
				for(HashMap<String, Object> map:lm){
					try {
						nodes = nodes+"\nsplit\n"+map.get("full").toString();
						} catch (Exception e) {
		...
		try{
			...
			ssIn.read(ssBu);
			ssIn.close();
			ss = new String(ssBu, "UTF-8");
			}catch(Exception e){
			
		}
		for(String xs:ss.split("\n")){
			hintsL.add(xs);
		}
       ...
       if(split[0].startsWith("-color:")){
			...
			linearVV.findViewById(R.id.lin);
			final TextView text = (TextView)
			linearVV.findViewById(R.id.text);
			final ImageView img = (ImageView)
			linearVV.findViewById(R.id.img);
			...
			if (split[0].startsWith("--")) {
				...
				linearVV.setOnClickListener(new View.OnClickListener(){
					@Override
					public void onClick(View _view){
						for (int i = ((((ViewGroup)_view.getParent()).indexOfChild(_view) + 1)); i < ((ViewGroup) (linear)).getChildCount(); i++) {
				...
				linearVV.setOnClickListener(new View.OnClickListener(){
					...
					public void onClick(View _view){
						...
						tm.schedule(tsk, 150);	nd.setText(split[0].replace("__star__if__",""));
		...
		if (!FileUtil.readFile(json_path).equals("")) {
			lm = new Gson().fromJson(FileUtil.readFile(json_path), new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
		...
		for(int x=0;x<frame.getChildCount();x++){
			...
			if(view instanceof Node){
				...
				try {
					...
					node.next = (Node)findViewById((int)nx);
					} catch (Exception e) {
				...
				if (node.is_else) {
					try {
						...
						node.true_node = (Node)findViewById((int)nx);
						} catch (Exception e) {
						
					}
					try {
						double nx = _toNumber(node.else_id);
						node.false_node = (Node)findViewById((int)nx);
						} catch (Exception e) {
		...
		frame.mScaleGestureDetector = new ScaleGestureDetector(getContext(), new ScaleGestureDetector.OnScaleGestureListener() {
	...
	public void showCode(){
	    ArrayList<HashMap<String, Object>> lm= new ArrayList<>();
				...
				if (!loopDetect) {
					...
					dl.setMessage(code);
					dl.setPositiveButton("Copy", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							((ClipboardManager) getContext().getSystemService(Context.CLIPBOARD_SERVICE)).setPrimaryClip(ClipData.newPlainText("clipboard", result));
						}
					});
	...
	class Node extends LinearLayout {
		public Node(Context ctx)  {
			...
			((ViewGroup) this).addView(thisVV);
			title = (TextView)	thisVV.findViewById(R.id.title);
			...
			id.setCornerRadii(new float[] { (10),(10),(10), (10), (10),(10),(10),(10) }); //LeftTop, //RightTop, //RightBottom, //LeftBottom,
			...
			androidx.core.widget.TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(false_text,(1),(100000),(1),androidx.core.widget.TextViewCompat.AUTO_SIZE_TEXT_TYPE_UNIFORM);
			androidx.core.widget.TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(true_text,(1),(100000),(1),androidx.core.widget.TextViewCompat.AUTO_SIZE_TEXT_TYPE_UNIFORM);
			androidx.core.widget.TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(title,(1),(100000),(1),androidx.core.widget.TextViewCompat.AUTO_SIZE_TEXT_TYPE_UNIFORM);
			setX(60);
			setY(60);
			...
			frame.addToC(cpath);
			_DragAble(this);
			right_circle.setOnTouchListener(new View.OnTouchListener(){
				@Override
			...
			true_circle.setOnTouchListener(new View.OnTouchListener(){
				@Override
				public boolean onTouch(View v, MotionEvent event){
					int ev = event.getAction();
					switch (ev) {
			...
			false_circle.setOnTouchListener(new View.OnTouchListener(){
				@Override
				public boolean onTouch(View v, MotionEvent event){
					int ev = event.getAction();
					switch (ev) {
		...
		public void updatePos(){
			try {
				...
				path.reset();
				cpath.reset();
				float rx = getRX(),ry = getRY(),lx=getLX(),ly=getLY(), tx = getTX(),ty = getTY(),fx = getFX(),fy=getFY();
				...
				if(true_node!=null){
					if(true_node.getParent()==null){
						...
						float ntx = true_node.getTX(),nty = true_node.getTY(),nfx = true_node.getFX(),nfy=true_node.getFY(),nx=true_node.getLX(),ny=true_node.getLY();
						path.moveTo(tx,ty);
				...
				if(false_node!=null){
					if(false_node.getParent()==null){
						...
						float ntx = false_node.getTX(),nty = false_node.getTY(),nfx = false_node.getFX(),nfy=false_node.getFY(),nx=false_node.getLX(),ny=false_node.getLY();
						path.moveTo(fx,fy);
       ...
       private int color=(int)Color.parseColor("#0078fe");
		...
		public void update(){
			...
			false_text.setPadding((0),(int)(Utils.convertPixelsToDp(getApplicationContext(), (3)) * scale),(0),(int)(Utils.convertPixelsToDp(getApplicationContext(), 3) * scale));
			container.setPadding((int)(Utils.convertPixelsToDp(getApplicationContext(), (8)) * scale),(Utils.convertPixelsToDp(getApplicationContext(), (int)(8 * scale))),(Utils.convertPixelsToDp(getApplicationContext(),(int)(8 * scale))),(Utils.convertPixelsToDp(getApplicationContext(), (int)(8 * scale))));
			_setHW(false_text, android.widget.LinearLayout.LayoutParams.MATCH_PARENT, Utils.convertPixelsToDp(getApplicationContext(), 25) * scale);
			_setHW(true_text, android.widget.LinearLayout.LayoutParams.MATCH_PARENT, Utils.convertPixelsToDp(getApplicationContext(), 25) * scale);
			true_text.setPadding((0),(int)(Utils.convertPixelsToDp(getApplicationContext(), (3)) * scale),(0),(int)(Utils.convertPixelsToDp(getApplicationContext(), 3) * scale));
			LinearLayout.LayoutParams params = new LinearLayout.LayoutParams( ViewGroup.LayoutParams.MATCH_PARENT,(int) (Utils.convertPixelsToDp(getContext(),35)*scale));
			int top =  0;
			...
			title.setPadding(Utils.convertPixelsToDp(getApplicationContext(),(int)(8 * scale)),(int)(Utils.convertPixelsToDp(getApplicationContext(), 8) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), 8) * scale),(Utils.convertPixelsToDp(getApplicationContext(), (int)(8 * scale))));
		...
		public void loadFrom(HashMap<String, Object> map){
			...
			try {
				if(this.is_else){	next_id=map.get("next_id").toString();
				} else next_id=map.get("next").toString();
			} catch(Exception ex){}
			try {	else_id=map.get("else_id").toString();
			} catch(Exception ex){}
			try {	if_id=map.get("next").toString();
			} catch(Exception ex){}
			...
			ArrayList<HashMap<String, Object>> lm = new Gson().fromJson(map.get("nf").toString(), new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
			for(HashMap<String, Object> hash:lm){
				ValueSetter nf= new ValueSetter(getContext());
				nf.node = this;
				try {
		...
		public HashMap<String, Object> getMap(){
			...
			map.put("next_id", true_text.getVisibility() == View.VISIBLE ? ((next==null)?"":next.getId()+"") : "null");
		...
		public String getCode(){
			...
			comes++; if(comes>frame.getChildCount()*10){
				_infiniteLoop();
				return "";
			}
			...
			String c="";
			try{ c=c+String.format(this.code,(Object[])s); }catch(Exception ex){}
			return c;
		}
	}
	...
	class ValueSetter extends LinearLayout {
		public ValueSetter(Context ctx)  {
			...
			((ViewGroup) this).addView(thisVV);
			value = (TextView) thisVV.findViewById(R.id.value);
			linear = (LinearLayout) thisVV.findViewById(R.id.linear);
			name = (TextView) thisVV.findViewById(R.id.name);
			androidx.core.widget.TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(value,(1),(100000),(1),androidx.core.widget.TextViewCompat.AUTO_SIZE_TEXT_TYPE_UNIFORM);
			androidx.core.widget.TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(name,(1),(100000),(1),androidx.core.widget.TextViewCompat.AUTO_SIZE_TEXT_TYPE_UNIFORM);
			update();
			this.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns(10, Color.TRANSPARENT));
			this.linear.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns(10, 0xFF1E1E1E));
			value.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns(10, 0xFFCFD8DC));
			value.setOnTouchListener(new OnTouchListener() {
				PointF DownPT = new PointF();
				PointF StartPT = new PointF();
				PointF DownPTR= new PointF();
			...
			if (isArabic) {
				...
				((ViewGroup) linear).addView(name);
			}
		}
		private TextView value,name;
		...
		public void update(){
			value.setPadding((int)(Utils.convertPixelsToDp(getApplicationContext(), (3)) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), (3)) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), (3)) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), (3)) * scale));
			{
				LinearLayout.LayoutParams params = new LinearLayout.LayoutParams((int)(Utils.convertPixelsToDp(getApplicationContext(), (70)) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), (25)) * scale));
				int top =  0;
				int bottom =  0;
				int right = (int)(Utils.convertPixelsToDp(getApplicationContext(), (8)) * scale);
				int left =  0;
				params.setMargins(left, top, right, bottom);
				
				value.setLayoutParams(params);
			}
			name.setPadding((int)(Utils.convertPixelsToDp(getApplicationContext(), (3)) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), (3)) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), (3)) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), (3)) * scale));
			linear.setPadding((int)(Utils.convertPixelsToDp(getApplicationContext(), (4)) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), (4)) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), (4)) * scale),(int)(Utils.convertPixelsToDp(getApplicationContext(), (4)) * scale));
			int len =name.getText().toString().length() *15;
			if(len<100) len = 100;
			name.setLayoutParams(new LinearLayout.LayoutParams((int) ((scale * Utils.convertPixelsToDp(getApplicationContext(), len))),(int) (Utils.convertPixelsToDp(getApplicationContext(), (25)) * scale)));
			
			LinearLayout.LayoutParams params = new LinearLayout.LayoutParams( ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
			{
				int top =  0;
				int bottom = (int) (Utils.convertPixelsToDp(getApplicationContext(),2) * scale);
				int right =  0;
				int left =  0;
				params.setMargins(left, top, right, bottom);
		...
		public void edit(){
			if (getName().equals("image")) {
				...
				cd = new AlertDialog.Builder(getContext(),android.R.style.Theme_DeviceDefault_NoActionBar_Fullscreen).create();
			...
			final AlertDialog cd = new AlertDialog.Builder(getContext(),android.R.style.Theme_DeviceDefault_NoActionBar_Fullscreen).create();
			...
			for (int ix = 0; ix < ((ViewGroup) linear).getChildCount(); ix++) {
				View vix = ((ViewGroup)linear).getChildAt(ix);
			...
			for(String ss:hintsL){
				...
				txt.setPadding((8),Utils.convertPixelsToDp(getApplicationContext(), 10),(2),(Utils.convertPixelsToDp(getApplicationContext(), 10)));
				if (ss.startsWith("-")) {
	...
	class Circle extends LinearLayout {
		...
		public Node node;
		public void update(){
			_setHW(this, Utils.convertPixelsToDp(getApplicationContext(), 90) * scale, Utils.convertPixelsToDp(getApplicationContext(), 15) * scale);
			_setHW(circle, Utils.convertPixelsToDp(getApplicationContext(), 15) * scale, Utils.convertPixelsToDp(getApplicationContext(), 15) * scale);
		}
			...
			@Override public boolean onTouch(View v, MotionEvent event) {
				switch (event.getAction()) {
					...
					if(dist<=Utils.convertPixelsToDp(getApplicationContext(), 20)){
						...
						if(((Node)_view).left_circle.getVisibility()==View.VISIBLE) _popupFor((Node)v);
					}
					break;
					default : break;
				}
		...
		public boolean onDrag(final View vm88,final DragEvent event) {
			...
			*/
			...
			switch(action) {
				...
				temp_path.cubicTo(x+100,y,event.getX()-100,event.getY(), event.getX(),event.getY());
				c_path.addCircle(event.getX(),event.getY(),sc.circle.getMeasuredWidth()/2,Path.Direction.CW);
				frame.invalidate();
				return true;
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView( int _position, View _v, ViewGroup _container) {
			...
			if (_data.get(_position).get("file").toString().equals("...")) {
				...
				textview1.setText(Uri.parse(_data.get(_position).get("file").toString()).getLastPathSegment());
				if (FileUtil.isFile(_data.get(_position).get("file").toString())) {
					linear1.setOnClickListener(new View.OnClickListener() {
						...
						public void onClick(View _view) {
							String p="";
							for(String s:interals) p = p+"/"+s;
							targetText.setText((p+"/").concat(Uri.parse(_data.get(_position).get("file").toString()).getLastPathSegment()));
							cd.dismiss();
						}
					});
					...
					linear1.setOnClickListener(new View.OnClickListener() {
						@Override
						public void onClick(View _view) {
							interals.add(Uri.parse(_data.get(_position).get("file").toString()).getLastPathSegment());
							_refreshList();
						}
					});
				}
